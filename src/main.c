...
int main (int argc, char **argv)
  ...
  if (program_context_parse_command_line (context, argc, argv))
    ...
    if (program_context_get_boolean (context, "scan", FALSE)
         || program_context_get_boolean (context, "quickscan", FALSE))
      {
      if (program_context_get (context, "index"))
        {
        log_info (NAME " scanner starting");

        ret = scanner_run (context);

        log_info (NAME " scanner finished");
        }
      }
    else
      {
