...
void path_append (Path *self, const char *name)
  ...
  if (wstring_length ((WString *)self) == 0)
    ...
    if (name[0] == PATH_SEPARATOR)
      wstring_append_printf ((WString *)self, "%s", name + 1);
    else
      wstring_append_printf ((WString *)self, "%s", name);
    }
  }
...
/*==========================================================================
  ...
  path_get_extension_utf8

*==========================================================================*/
UTF8 *path_get_extension_utf8 (const Path *self)
  {
  LOG_IN
  UTF8 *ret = NULL;

  const UTF32 *s = wstring_cstr ((WString *)self);
  int seppos = wstring_find_last ((WString *)self, path_separator()); 

  if (seppos < 0) seppos = 0;  // if no /, assume start of path string
  int l = wstring_length_utf32 (s);
  for (int i = l - 1; i > seppos && !ret; i--)
    {
    UTF32 c = s[i];
    if (c == '.') 
      {
      ret = string_utf32_to_utf8 (s + i + 1); 
      }
    }
