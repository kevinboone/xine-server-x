...
BOOL program_context_parse_command_line (ProgramContext *self, 
  ...
  static struct option long_options[] =
    {
      ...
      {"xsport", required_argument, NULL, 0},
      {"xshost", required_argument, NULL, 0},
      {"xslaunch", required_argument, NULL, 'x'},
      {"gxsradio", required_argument, NULL, 'g'},
   ...
   while (ret)
     {
     int option_index = 0;
     opt = getopt_long (argc, argv, "hvl:r:p:x:dr:i:sq",
     ...
     switch (opt)
       ...
       case 0:
         if (strcmp (long_options[option_index].name, "help") == 0)
           ...
           program_context_put_boolean (self, "scan", TRUE);
         else if (strcmp (long_options[option_index].name, "quickscan") == 0)
           program_context_put_boolean (self, "quickscan", TRUE);
         ...
         else if (strcmp (long_options[option_index].name, "xsport") == 0)
           program_context_put_integer (self, "xsport", atoi (optarg)); 
         else if (strcmp (long_options[option_index].name, "root") == 0)
           program_context_put (self, "root", optarg); 
         else if (strcmp (long_options[option_index].name, "xshost") == 0)
           program_context_put (self, "xshost", optarg); 
         else if (strcmp (long_options[option_index].name, "gxsradio") == 0)
           program_context_put (self, "gxsradio", optarg); 
         else if (strcmp (long_options[option_index].name, "xslaunch") == 0)
           program_context_put (self, "xslaunch", optarg); 
         else if (strcmp (long_options[option_index].name, "index") == 0)
           program_context_put (self, "index", optarg); 
         else
           exit (-1);
         break;
       case 'h': case '?': 
       ...
       case 's': 
         program_context_put_boolean (self, "scan", TRUE); break;
       case 'q': 
         program_context_put_boolean (self, "quickscan", TRUE); break;
       case 'v': 
       ...
       case 'p': program_context_put_integer (self, "port", 
           atoi (optarg)); break;
       case 'r': program_context_put (self, "root", optarg);  break;
       case 'x': program_context_put (self, "xslaunch", optarg);  break;
       case 'g': program_context_put (self, "gxsradio", optarg);  break;
       case 'i': program_context_put (self, "index", optarg);  break;
       default:
         ret = FALSE; 
       }
    }
  ...
  if (program_context_get_boolean (self, "show-version", FALSE))
   ...
   const char *root = program_context_get (self, "root");
   if (root)
    {
    if (!file_is_directory (root))
      {
      log_error ("Media root %s is not a valid directory", root);
      ret = FALSE;
      }
    }
   else
    {
    log_error ("Media root directory must be specified, "
