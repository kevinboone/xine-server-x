...
// Returns TRUE if the file extension is used by audio files that are generally
//   playable by Xine. Xine itself does not care about file extensions, and
//   a file might play even if the extension is unconventional. This 
//   function does not require a round-trip to the server
BOOL   xineserver_is_playable_ext (const char *ext);
